...
export default function ExerciseWorkspace({ exercises }: { exercises: Exercise[] }) {
  ...
  return <section className="exercise-panel">
    ...
    {visible.length ? <div className="exercise-grid">{visible.map((exercise) => <Link className="exercise-card" href={`/exercise/${exercise.id}`} key={exercise.id}><span className="exercise-number">{exercise.exerciseNumber}</span><span>{exercise.language === "ta" ? "தமிழ் பயிற்சி" : "English exercise"}</span><span className="row-arrow">→</span></Link>)}</div> : <div className="exercise-empty"><div className="empty-icon">∑</div><strong>No exercise labels found for this chapter</strong><p>Check that the Neon database has been migrated and seeded.</p></div>}
  </section>;
}
